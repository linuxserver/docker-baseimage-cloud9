    ...
    stage("Set ENV Variables base"){
      steps{
        script{
          env.EXIT_STATUS = ''
          env.LS_RELEASE = sh(
            script: '''docker run --rm ghcr.io/linuxserver/alexeiled-skopeo sh -c 'skopeo inspect docker://docker.io/'${DOCKERHUB_IMAGE}':files 2>/dev/null' | jq -r '.Labels.build_version' | awk '{print $3}' | grep '\\-ls' || : ''',
    ...
    stage('Update-Templates') {
      ...
      steps {
        sh '''#! /bin/bash
              ...
              docker run --rm -e CONTAINER_NAME=${CONTAINER_NAME} -e GITHUB_BRANCH=files -v ${TEMPDIR}:/ansible/jenkins ghcr.io/linuxserver/jenkins-builder:latest 
          ...
          retry(5) {
            sh '''#! /bin/bash
                  ...
                  for PUSHIMAGE in "${GITHUBIMAGE}" "${GITLABIMAGE}" "${IMAGE}"; do
                    ...
                    docker tag ${PUSHIMAGE}:${META_TAG} ${PUSHIMAGE}:files
                    docker tag ${PUSHIMAGE}:${META_TAG} ${PUSHIMAGE}:${EXT_RELEASE_TAG}
                    docker push ${PUSHIMAGE}:files
          ...
          retry(5) {
            sh '''#! /bin/bash
                  ...
                  for MANIFESTIMAGE in "${IMAGE}" "${GITLABIMAGE}" "${GITHUBIMAGE}"; do
                    ...
                    docker tag ${MANIFESTIMAGE}:amd64-${META_TAG} ${MANIFESTIMAGE}:amd64-files
                    docker tag ${MANIFESTIMAGE}:arm32v7-${META_TAG} ${MANIFESTIMAGE}:arm32v7-files
                    docker tag ${MANIFESTIMAGE}:arm64v8-${META_TAG} ${MANIFESTIMAGE}:arm64v8-files
                    ...
                    docker push ${MANIFESTIMAGE}:amd64-files
                    docker push ${MANIFESTIMAGE}:arm32v7-files
                    docker push ${MANIFESTIMAGE}:arm64v8-files
                    docker push ${MANIFESTIMAGE}:amd64-${EXT_RELEASE_TAG}
                    docker push ${MANIFESTIMAGE}:arm32v7-${EXT_RELEASE_TAG}
                    docker push ${MANIFESTIMAGE}:arm64v8-${EXT_RELEASE_TAG}
                    docker manifest push --purge ${MANIFESTIMAGE}:files || :
                    docker manifest create ${MANIFESTIMAGE}:files ${MANIFESTIMAGE}:amd64-files ${MANIFESTIMAGE}:arm32v7-files ${MANIFESTIMAGE}:arm64v8-files
                    docker manifest annotate ${MANIFESTIMAGE}:files ${MANIFESTIMAGE}:arm32v7-files --os linux --arch arm
                    docker manifest annotate ${MANIFESTIMAGE}:files ${MANIFESTIMAGE}:arm64v8-files --os linux --arch arm64 --variant v8
